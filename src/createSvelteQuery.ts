...
const createSvelteQuery = (client: ReturnType<typeof create> = create(), options: { name?: string } = {}) => {
  ...
  return {
    ...
    createDistricts: (provinceCode?: string) => createQuery(initial(validkey(provinceCode, key.ds), fetcher.ds, iskey(provinceCode))),
    createDistrict: (code?: string) => createQuery(initial(validkey(code, key.d), fetcher.d, iskey(code))),
    createSubdistricts: (districtCode?: string) => createQuery(initial(validkey(districtCode, key.ss), fetcher.ss, iskey(districtCode))),
